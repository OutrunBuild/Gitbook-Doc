...
* **FFLaunch**：为社区提供项目融资和空投分发工具，为投资者提供免费无风险获得项目代币的机会
  * 标准化发行流程：Pro / 公开无许可两种模式，Pro 模式需要 Outrun 团队技术支持。
  * 空投/FAirdrop 工具：支持白名单模式，仅限白名单用户可以参与创世
  * 上线节奏：为 Pr o模式的项目设置生态主题位（DeFi / 工具 / 社交 / AI）做集中曝光
  * 合作分发：对接钱包发现页、Launchpad 聚合器、任务平台（QuestN / Galxe / Layer3等）联动分发
  * 风控合规：Pro 模式需要风控，发行模板审查、团队信息披露清单、Rug 风险条款、KYC/审计可选项
...
* **防女巫与质量控制**
  ...
  * 项目准入（FFLaunch Pro 模式）：发行模板审查、团队披露清单、Rug 风险条款、外部审计/KYC 可选
* **供给侧运营（Memeverse/FFLaunch 管道)**
  * FFLaunch Pro（经筛选与支持）：每月 ≥ 1 个“FFLaunch Pro 创世”，享主题位与联合宣发支持
    * 项目管道与配额：来源（公链基金会/钱包/安全公司/黑客松/社区提案），品类配比（DeFi/工具/社交/AI），区域配比与新链配额
    * 立项与基本检查（FFLaunch Pro）：团队与合约基础信息、发行参数自查清单、到期赎回与费用说明的一致性校验
    * 启动 SOP（T-7 → T0）：
      * T-7：提交发行参数（总量、锁定期、硬顶、白名单或公开），创世页自动生成预览
      * T-5：完成任务与积分倍率配置、推荐归因、完成自测清单（交互流程/结算说明/FAQ）
      * T-3：小流量预演与公告排期、素材上线（海报/线程模板）、AMA多语种发布脚本
...
* **收益对比营销**：制作与竞品的收益对比内容
* **教育内容**：制作 OutStake 以及复杂 DeFi 策略的使用教程
...
2. **产品指标**
   ...
   * 推荐成交占比（OutSwap）、MEV Guard 覆盖率与被夹率，被狙击率下降
   * POL 借贷：借贷量、利息收入、借贷利用率、杠杆创世渗透率
3. **社区指标**
...
2. **变量与处理（Treatment）**
   * 可控变量：积分倍率、推荐佣金、LP 激励、杠杆借贷利率、创世主题与曝光、默认 UPT 路径
...
10. **知识库与复用**
    ...
    * 快速复用：公共资产库（积分/佣金/参数表、任务与公告模板），一键引用到新实验
...
11. **实验模板（示例）**
    ...
    * 创世主题周实验：目标=创世规模↑；处理=“AI/工具”主题页 + 曝光位；Primary=创世资金规模；Guardrail=创世失败率
12. **排期与并发**
