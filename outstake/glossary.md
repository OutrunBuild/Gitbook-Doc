...
不同于其他协议，OutStake 的 UPT 是通用 ERC20 代币，没有过期时间。同时质押相同资产类型的原生收益代币会铸造同一种 UPT，简单讲质押 stETH，rETH，stone 会铸造同一种 UPT 代币（相同代币合约地址），而非三种 PT 代币。

### **YT = 收益代币 (Yield Token)**

YT 是由原生收益代币分拆出来的收益的组成部份。持有 YT 代表你拥有底层资产产生的所有收益，并且可以随时燃烧 YT 赎回当前收益池已积累的原生收益。

不同于其他协议，OutStake 的 YT 是通用 ERC20 代币，没有过期时间，不同质押时间所铸造的 YT 也是同一种代币（相同代币合约地址），与 UPT 不同，每一种原生收益代币都会铸造对应的 YT。
...
POT 是一种基于 [ERC1155](https://eips.ethereum.org/EIPS/eip-1155) 标准的用于封装用户质押仓位的期权代币，持有 POT 代表你握有仓位的到期赎回权，在 POT 锁定时间到期后，可以燃烧 POT 与对应数量的 UPT 从而赎回质押的原生收益代币本金部分。由于 POT 是 [ERC1155](https://eips.ethereum.org/EIPS/eip-1155) 标准的，所以支持仓位的部分赎回。

POT 记录了仓位的原生收益代币质押数量，恒定本金价值以及铸造的 UPT 数量，因此其到期后燃烧的 POT 份额以及 UPT 数量与所赎回的本金价值是一个固定的关系，从而形成固定的利率，通过交易 POT 可以交易获得固定利率的权利。
